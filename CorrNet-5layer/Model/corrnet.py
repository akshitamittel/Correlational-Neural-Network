...
class CorrNet(object):
    ...
    def init(self, numpy_rng, theano_rng=None, l_rate=0.01, optimization="sgd",
             tied=False, n_visible_left=None, n_visible_right=None, n_hidden=None, n_hidden_mid=None,
             lamda=5, W_left=None, W_right=None, b=None, W_mid=None, b_mid=None, W_mid_prime=None, b_prime=None,
             W_left_prime=None, W_right_prime=None, b_prime_left=None, b_prime_right=None, 
             input_left=None, input_right=None, hidden_activation="sigmoid", output_activation="sigmoid", 
             hidden_mid_activation="sigmoid", hidden_activation_prime="sigmoid", loss_fn = "squarrederror",
             op_folder=None):
        ...
        # 2. Add number of neurons in extra hidden layer
        self.n_visible_left = n_visible_left
        self.n_visible_right = n_visible_right
        self.n_hidden = n_hidden
        self.n_hidden_mid = n_hidden_mid
  
        self.lamda = lamda
        self.hidden_activation = hidden_activation
        self.hidden_mid_activation = hidden_mid_activation
        ...
        self.W_mid = self.Initializer.fan_based_sigmoid("W_mid", W_mid, n_hidden, n_hidden_mid)
        self.optimizer.register_variable("W_mid",n_hidden,n_hidden_mid)
        ...
        if not tied:
            ...
            self.W_mid_prime = self.Initializer.fan_based_sigmoid("W_mid_prime", W_mid_prime, n_hidden_mid, n_hidden)
            self.optimizer.register_variable("W_mid_prime",n_hidden_mid,n_hidden)
        else:
            self.W_left_prime = self.W_left.T
            self.W_right_prime = self.W_right.T
            self.W_mid_prime = self.W_mid.T

        #4. Add hidden to zero_vector
        self.b = self.Initializer.zero_vector("b", b, n_hidden)
        self.optimizer.register_variable("b",1,n_hidden)

        self.b_mid = self.Initializer.zero_vector("b_mid", b_mid, n_hidden_mid)
        self.optimizer.register_variable("b_mid",1,n_hidden_mid)

        self.b_prime = self.Initializer.zero_vector("b_prime", b_prime, n_hidden)
        self.optimizer.register_variable("b_prime",1,n_hidden)
        ...
        if tied:
            self.params = [self.W_left, self.W_right, self.W_mid, self.b, self.b_mid, self.b_prime, self.b_prime_left, self.b_prime_right]
            self.param_names = ["W_left", "W_right","W_mid", "b","b_mid", "b_prime", "b_prime_left", "b_prime_right"]
        else:
            self.params = [self.W_left, self.W_right, self.W_mid, self.b, self.b_mid, self.b_prime, self.b_prime_left, self.b_prime_right, self.W_mid_prime, self.W_left_prime, self.W_right_prime]
            self.param_names = ["W_left", "W_right","W_mid", "b","b_mid", "b_prime", "b_prime_left", "b_prime_right","W_mid_prime", "W_left_prime", "W_right_prime"]
    ...
    def train_common(self,mtype="1111"):
        ...
        y1_2ndpre = T.dot(y1, self.W_mid) + self.b_mid
        y1_2nd = activation(y1_2ndpre, self.hidden_mid_activation)

        y1_3rdpre = T.dot(y1_2nd, self.W_mid_prime) + self.b_prime
        y1_3rd = activation(y1_3rdpre, self.hidden_activation_prime)

        z1_left_pre = T.dot(y1_3rd, self.W_left_prime) + self.b_prime_left
        z1_right_pre = T.dot(y1_3rd,self.W_right_prime) + self.b_prime_right
        ...
        y2_2ndpre = T.dot(y2, self.W_mid) + self.b_mid
        y2_2nd = activation(y2_2ndpre, self.hidden_mid_activation)

        y2_3rdpre = T.dot(y2_2nd, self.W_mid_prime) + self.b_prime
        y2_3rd = activation(y2_3rdpre, self.hidden_activation_prime)

        z2_left_pre = T.dot(y2_3rd, self.W_left_prime) + self.b_prime_left
        z2_right_pre = T.dot(y2_3rd,self.W_right_prime) + self.b_prime_right
        ...
        y3_2ndpre = T.dot(y3, self.W_mid) + self.b_mid
        y3_2nd = activation(y3_2ndpre, self.hidden_mid_activation)

        y3_3rdpre = T.dot(y3_2nd, self.W_mid_prime) + self.b_prime
        y3_3rd = activation(y3_3rdpre, self.hidden_activation_prime)

        z3_left_pre = T.dot(y3_3rd, self.W_left_prime) + self.b_prime_left
        z3_right_pre = T.dot(y3_3rd,self.W_right_prime) + self.b_prime_right
        ...
        # Calculating L4: TODO: Ask Debaditya Bhaiyaa. (y1, y1_2nd, or y1_3rd)
        y1_mean = T.mean(y1_3rd, axis=0)
        y1_centered = y1_3rd - y1_mean
        y2_mean = T.mean(y2_3rd, axis=0)
        y2_centered = y2_3rd - y2_mean
    ...
    def train_left(self):
        ...
        v_pre = T.dot(y, self.W_mid) + self.b_mid
        v = activation(v_pre, self.hidden_mid_activation)

        v1_pre = T.dot(v, self.W_mid_prime) + self.b_prime
        v1 = activation(v1_pre, self.hidden_activation_prime)

        z_left_pre = T.dot(v1, self.W_left_prime) + self.b_prime_left
        ...
        if self.tied:
            curr_params = [self.W_left, self.b, self.W_mid, self.b_mid, self.b_prime, self.b_prime_left]
            curr_param_names = ["W_left", "b","W_mid", "b_mid", "b_prime", "b_prime_left"]
        else:
            curr_params = [self.W_left, self.b, self.W_mid, self.b_mid, self.b_prime, self.b_prime_left, self.W_mid_prime, self.W_left_prime]
            curr_param_names = ["W_left", "b","W_mid", "b_mid", "b_prime", "b_prime_left","W_mid_prime", "W_left_prime"]
    ...
    def train_right(self):
        ...
        z_right_pre = T.dot(v1, self.W_right_prime) + self.b_prime_right
        z_right = activation(z_right_pre, self.output_activation)
        L = loss(z_right, self.x_right, self.loss_fn)
        cost = T.mean(L)

        if self.tied:
            curr_params = [self.W_right, self.b, self.W_mid, self.b_mid, self.b_prime, self.b_prime_right]
            curr_param_names = ["W_right", "b","W_mid", "b_mid", "b_prime", "b_prime_right"]
        else:
            curr_params = [self.W_right, self.b, self.W_mid, self.b_mid, self.b_prime, self.b_prime_right, self.W_mid_prime, self.W_right_prime]
            curr_param_names = ["W_right", "b","W_mid", "b_mid", "b_prime", "b_prime_right","W_mid_prime", "W_right_prime"]
    ...
    def reconstruct_from_left(self):
        ...
        v1_pre = T.dot(y, self.W_mid) + self.b_mid
        v1 = activation(v1_pre, self.hidden_mid_activation)

        v2_pre = T.dot(v1, self.W_mid_prime)
        v2 = activation(v2_pre, self.hidden_activation_prime)

        z_left_pre = T.dot(v2, self.W_left_prime) + self.b_prime_left
        z_right_pre = T.dot(v2,self.W_right_prime) + self.b_prime_right
        z_left = activation(z_left_pre, self.output_activation)
        z_right = activation(z_right_pre, self.output_activation)
    ...
    def save_params(self):
        ...
        params["n_hidden_mid"] = self.n_hidden_mid
        params["lamda"] = self.lamda
        params["hidden_activation"] = self.hidden_activation
        params["hidden_activation_prime"] = self.hidden_activation_prime
        params["hidden_mid_activation"] = self.hidden_mid_activation
    ...
    def load(self, folder, input_left=None, input_right=None):
        ...
        self.init(plist["numpy_rng"], theano_rng=plist["theano_rng"], l_rate=plist["l_rate"],
                  ...
                  n_hidden=plist["n_hidden"],n_hidden_mid=plist["n_hidden_mid"], lamda=plist["lamda"], 
                  W_left=folder+"W_left", W_right=folder+"W_right", b=folder+"b", W_mid=folder+"W_mid", 
                  b_mid=folder+"b_mid", W_mid_prime=folder+"W_mid_prime", b_prime=folder+"b_prime", 
                  W_left_prime=folder+"W_left_prime", W_right_prime=folder+"W_right_prime", 
                  b_prime_left=folder+"b_prime_left", b_prime_right=folder+"b_prime_right", 
                  input_left=input_left, input_right=input_right, hidden_activation=plist["hidden_activation"], 
                  hidden_mid_activation=plist["hidden_mid_activation"], output_activation=plist["output_activation"],
                  hidden_activation_prime = plist["hidden_activation_prime"], loss_fn = plist["loss_fn"], op_folder=folder)
...
def trainCorrNet(src_folder, tgt_folder, batch_size = 20, training_epochs=40,
                 ...
                 n_visible_right=None, n_hidden=None, n_hidden_mid=None, lamda=5,
                 W_left=None, W_right=None, b=None, W_mid=None, b_mid=None, W_mid_prime=None,
                 b_prime=None, W_left_prime=None, W_right_prime=None,
                 b_prime_left=None, b_prime_right=None, hidden_activation="sigmoid",
                 output_activation="sigmoid", hidden_mid_activation="sigmoid", 
                 hidden_activation_prime="sigmoid", loss_fn = "squarrederror"):
    ...
    # 8. Add hidden params
    model = CorrNet()
    model.init(numpy_rng=rng, theano_rng=theano_rng, l_rate=l_rate, optimization=optimization, tied=tied, n_visible_left=n_visible_left, n_visible_right=n_visible_right, n_hidden=n_hidden, n_hidden_mid=n_hidden_mid, lamda=lamda, W_left=W_left, W_right=W_right, b=b, W_mid=W_mid, b_mid=b_mid, W_mid_prime=W_mid_prime, b_prime=b_prime, W_left_prime=W_left_prime, W_right_prime=W_right_prime, b_prime_left=b_prime_left, b_prime_right=b_prime_right, input_left=x_left, input_right=x_right, hidden_activation=hidden_activation, hidden_mid_activation=hidden_mid_activation, hidden_activation_prime=hidden_activation_prime, output_activation=output_activation, loss_fn =loss_fn, op_folder=tgt_folder)
